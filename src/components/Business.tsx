  ...
  return (
    <div className={`flex flex-row p-6 rounded-[20px] ${props.index !== features.length -1 ? "mb-6" : "mb-0"} feature-card`}>
      <div className={`w-[64x] h-[64px] rounded-full ${styles.flexCenter}`}>
        <img src={props.icon} alt="icon" className='w-[25%] h-[50%] object-contain'/>
        <div className='flex flex-col ml-3'>
          <h4 className='font-poppins font-semibold text-white text-[18px] leading-[23px] mb-1'>{props.title}</h4>
          <p className='font-poppins font-normal text-dimWhite text-[16px] leading-[24Ipx] mb-1'>{props.content}</p>
        </div>
...
const Business = () => {
  return (
    <section id='features' className={layout.section}>
      <div className={`flex md:flex-col flex-col ${styles.paddingY}`}>
